...
2. **Input:** A mapping of anonymised firm IDs and their historical archived URLs.
3. **Processing:** - Domain parsing via `urllib.parse`.
    - Boolean query construction using a specialised linguistic dictionary.
...
- First, ensure you've run the anonymisation script `anonymize_data.py` if using real data. Then, execute the main harvest script `SERC_harvest_ml.py`
...
### 5. Limitations
- **API Quotas:** The free tier of Google Custom Search API is limited to ~100 queries per day. For larger datasets, consider the paid tier, multiple API keys, or alternative search APIs like Serper.dev and Brave Search API.
